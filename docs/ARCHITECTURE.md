...
An Express application exposes a JSON REST API backed by MongoDB, plus a Swagger UI for exploring and calling it interactively. A React client (`frontend-client/react-ts/`, documented separately in [its own README](../frontend-client/react-ts/README.md)) consumes this API; the two are developed and deployed independently, with the frontend's dev server proxying `/api` requests to this backend.

```mermaid
flowchart TD
    Client["Client"] -->|"HTTP (JSON)"| App["Express app (src/app.ts)"]
    App -->|"cors → express.json() → /api/docs (Swagger UI) → routes → 404 handler → errorHandler"| Routers["Routers (src/routes/*.ts)"]
...
The app is split into `src/app.ts` (`createApp()`, pure Express wiring — no side effects, so it can be imported by tests without opening a port or DB connection) and `src/server.ts` (the actual process entry point). In order, `server.ts`:

1. Loads environment variables (`dotenv.config()`) — must happen before any other import touches `process.env`.
2. Calls `createApp()`, which configures `cors()`, `express.json()`, mounts the Swagger UI (`/api/docs`) and raw spec (`/api/docs.json`), the `/api/health` check, then every router under `/api`, followed by a 404 handler and the centralized `errorHandler`.
3. Connects to MongoDB (`connectDB()`) and only then starts listening.
...
`src/swagger.ts` builds an OpenAPI 3.0 spec with `swagger-jsdoc`, scanning `@openapi` JSDoc blocks directly above each route handler in `src/routes/*.ts` (and the health check in `app.ts`) — the spec is generated from the same source as the implementation, not maintained separately. `swagger-ui-express` serves an interactive UI from that spec at `/api/docs`; the raw JSON is at `/api/docs.json`. The UI has a **Authorize** button for pasting a bearer token (from `POST /api/auth/login`) so protected routes can be called directly from the browser.

This generated reference is the source of truth for exact request/response shapes; [`DESIGN.md`](DESIGN.md#api-reference) covers the same routes at a narrative level (why they're shaped the way they are), not as a byte-for-byte duplicate.
...
sequenceDiagram
    ...
    participant R as Protected route (e.g. /api/tasks)
    ...
    C->>R: GET /tasks (Authorization: Bearer token)
...
flowchart LR
    subgraph "docker-compose.yml (dev)"
        B1["Browser :3000"] -->|HTTP| C1["client (nginx)"] -->|"/api/*"| A1["api :4000"]
    end
...
`eks/` is a separate, AWS-oriented set of manifests for the same three pieces (also in a `task-tracker` namespace) — not applied on top of `k8s/`, but an alternative to it; see the root [README](../README.md#running-on-eks) for the full walkthrough. The meaningful differences from `k8s/`:

- **Images come from ECR**, not the local Docker image store — `frontend-client-svc`/`backend-api` Deployments reference `<account-id>.dkr.ecr.<region>.amazonaws.com/...` images, pulled via an `ecr-registry` `docker-registry` Secret created from a short-lived `aws ecr get-login-password` token rather than `imagePullPolicy: Never`.
- **The client Service is `type: LoadBalancer`**, not `NodePort` — on EKS this provisions a real AWS load balancer with a public hostname, versus Docker Desktop's `localhost:30080` NodePort mapping.
- **Mongo has no PVC — `eks/mongo-deployment.yml` mounts an `emptyDir`.** Data does not survive a pod restart or reschedule on EKS; this is a deliberate simplification for a demo/capstone cluster, not a production pattern (`k8s/`'s `mongo.yaml`, by contrast, does use a `ReadWriteOnce` PVC). `Scripts/fix-ecr-cross-account.sh` and `Scripts/ecr-consumer-pull-policy.json` (documented in the root README) support the cross-account ECR pull case, where the cluster and the image registry live in different AWS accounts.

## CI pipeline

`.github/workflows/ci.yml` runs on every push/PR touching `backend-api/**`, `frontend-client/react-ts/**`, or the workflow file itself:

```mermaid
flowchart LR
    subgraph "backend-tests"
        B1["npm ci"] --> B2["typecheck"] --> B3["test (unit + DB-backed)"] --> B4["test:integration"]
    end
    subgraph "frontend-tests"
        F1["npm ci"] --> F2["typecheck"] --> F3["lint"] --> F4["build"] --> F5["test"]
    end
    B4 --> D["docker-build:<br/>build backend-api/Dockerfile<br/>+ frontend-client/Dockerfile<br/>(push: false)"]
    F5 --> D
```

`backend-tests` runs against a real `mongo:7` service container (with a health-check gate so tests don't start against a Mongo that isn't accepting connections yet) rather than mocking the database — consistent with the DB-backed model tests described below. `docker-build` only builds (`push: false`) to catch a broken Dockerfile early; it deliberately depends on both test jobs passing first, so a bad image is never built from code that doesn't even pass its own tests. A separate `pages.yml` workflow (see the root [README](../README.md#build-status)) deploys `docs/` to GitHub Pages on pushes to `main`, independent of this pipeline.
